...
function UILib.Window(titleA, titleB, gameName)
    ...
    local function applyFade()
        if minimized then
            for _,d in ipairs(allDrawings) do
                if d ~= dScrollBg and d ~= dScrollThumb then d.Visible=false end
            end
            return
        end
        ...
        for _,d in ipairs(allDrawings) do
            if d == dScrollBg or d == dScrollThumb then
                -- skip; scrollbar visibility is managed separately
            elseif showSet[d] then
    ...
    local function restoreFullMenu()
        ...
        showMiniUI(false)
        for _,d in ipairs(allDrawings) do 
            if d ~= dScrollThumb and d ~= dScrollBg then
                d.Visible=false 
            end
        end
        for _,d in ipairs(allDrawings) do tabSet[d]=nil end
    ...
    function win:Init(defaultTab, charLabelFn, notifFn)
        ...
        dScrollBg = mkD(mkSq(uiX+L.W-6,uiY+L.TOPBAR+2,4,L.H-L.TOPBAR-L.FOOTER-4,Color3.fromRGB(18,20,28),true,1,4,nil,2))
        dScrollThumb = mkD(mkSq(uiX+L.W-6,uiY+L.TOPBAR+2,4,20,C.ACCENT,true,1,5,nil,2))
