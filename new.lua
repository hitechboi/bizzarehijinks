...
function UILib.Window(titleA, titleB, gameName)
    ...
    function win:Init(defaultTab, charLabelFn, notifFn)
        ...
        spawn(function()
            ...
            dBg.Filled=true; dBg.ZIndex=15; dBg.Color=Color3.fromRGB(15, 15, 20)
            
            local dTxt = Drawing.new("Text")
            dTxt.Size=16; dTxt.Color=C.ACCENT; dTxt.Center=true; dTxt.Outline=true; dTxt.ZIndex=16
            pcall(function() dTxt.Font=Drawing.Fonts.UI end)
            
            local t0 = tick()
            while tick() - t0 < 1.0 and not destroyed do
                task.wait()
                local dots = string.rep(".", math.floor((tick()*3)%4))
                dTxt.Text = "Loading " .. (gameName or "UI") .. dots
            ...
            local fade = tick()
            while tick() - fade < 0.3 and not destroyed do
                task.wait()
