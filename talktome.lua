                        ...
                        if b.tab==currentTab and not b.isSlider and showSet[b.bg] then
                            if inBox(uiX+b.rx,uiY+(b.currentRY or b.ry),b.cw,b.ch) then
                                ...
                                elseif b.isDiv and b.collapsible and b.sectionName then
                                    ...
                                    -- Recalculate layout & animation targets top-to-bottom
                                    local currentY = 6
                                    local activeSection = nil
                                    local activeSectionY = nil
                                    local sectionIsCollapsed = false
                                    
                                    for _, cb2 in ipairs(btns) do
                                        if cb2.tab == currentTab then
                                            if cb2.isDiv then
                                                activeSection = cb2.sectionName
                                                sectionIsCollapsed = activeSection and _collapseSections[activeSection]
                                                activeSectionY = L.TOPBAR + currentY
                                                
                                                cb2.ry = L.TOPBAR + currentY
                                                currentY = currentY + cb2.ch + 2
                                                cb2._collapsing = false
                                                bShow(cb2, true)
                                            else
                                                if sectionIsCollapsed then
                                                    -- Slide up into the division header
                                                    cb2._collapsing = true
                                                    cb2._collapseTarget = activeSectionY + 14
                                                    -- Do NOT bShow(cb2, false) here, let the main loop hide it when animation finishes
                                                else
                                                    -- Slated to uncollapse: start animating from the header, slide down to normal position
                                                    ...
                                                    local h = cb2.ch + 2
                                                    if cb2.isSlider then h = h + 6 end
                                                    currentY = currentY + h
                                                end
                                            end
                                        end
                                    ...
                                    -- Update the max scroll area and uiTargetH based on the newly calculated layout size
                                    tabRowY[currentTab] = currentY + 10
