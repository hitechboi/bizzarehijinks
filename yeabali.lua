...
function UILib.Window(titleA, titleB, gameName)
    ...
    local function switchTab(name)
        ...
        for _,b in ipairs(btns) do
            if b.tab==name then
                ...
                b.ry=b.baseRY; b.currentRY=b.baseRY; b._collapsing=false; b._collapseTarget=nil
    ...
    local function updatePos()
        ...
        dTitleA.Position  =Vector2.new(uiX+14+(#titleA*8)+6,uiY+12)
        dTitleG.Position  =Vector2.new(uiX+14+(#titleA*8)+6+(#titleB*8)+12,uiY+12)
        ...
        dCharLbl.Position =Vector2.new(uiX+10,uiY+curH-L.FOOTER+5)
    ...
    function win:Init(defaultTab, charLabelFn, notifFn)
        ...
        dCharLbl = mkD(mkTx("Matcha VM | v1.6.0",uiX+10,uiY+L.H-L.FOOTER+5,10,C.GRAY,false,9))
        ...
        spawn(function()
            ...
            for _, stage in ipairs(progressStages) do
                -- Lerp towards the target chunk over the specified delay
        ...
        while not destroyed do
            ...
            if not minimized then
                ...
                if openDropdown then
                    for i,o in ipairs(openDropdown.optBgs) do
                        ...
                        if o.bg.Visible then
                            if inBox(uiX+openDropdown.rx, uiY+o.ry, openDropdown.cw, openDropdown.ch) then
                        ...
                        if b.tab==currentTab and not b.isSlider and showSet[b.bg] then
                            if inBox(uiX+b.rx,uiY+(b.currentRY or b.ry),b.cw,b.ch) then
                                ...
                                elseif b.isDiv and b.collapsible and b.sectionName then
                                    -- Close any open dropdowns to prevent layout desync during shift
                                    if openDropdown then
                                    ...
                                    -- Recalculate layout using baseRY difference (preserves flawless pixel padding)
                                    local currentYOffset = 0
                                    local activeSectionY = 0
                                    local sectionIsCollapsed = false
                                    
                                    for i, cb2 in ipairs(btns) do
                                        if cb2.tab == currentTab then
                                            if cb2.isDiv then
                                                sectionIsCollapsed = cb2.collapsible and _collapseSections[cb2.sectionName]
                                                cb2.ry = cb2.baseRY - currentYOffset
                                                bShow(cb2, true)
                                            else
                                                if sectionIsCollapsed then
                                                    -- Hide instantly to prevent unmasked overlapping overlap and phantom hitboxes
                                                    cb2._collapsing = false
                                                    cb2._collapseTarget = nil
                                                    bShow(cb2, false)
                                                    
                                                    -- Track the hidden offset purely mathematically
                                                    local nextBaseRY = cb2.baseRY + cb2.ch + 6
                                                    for j = i + 1, #btns do
                                                        if btns[j].tab == currentTab then
                                                            nextBaseRY = btns[j].baseRY
                                                            break
                                                        end
                                                    end
                                                    currentYOffset = currentYOffset + (nextBaseRY - cb2.baseRY)
                                                    
                                                else
                                                    -- Expanding or natively visible
                                                    -- Ensure its target is accurately set mathematically so it accords into the newly opened gap smoothly
                                                    cb2._collapsing = false
                                                    cb2._collapseTarget = nil
                                                    cb2.ry = cb2.baseRY - currentYOffset
                                                    
                                                    -- If it was snapped off-layout, jump its origin into the parent section so it slides ALONG with the accordion
                                                    if not showSet[cb2.bg] then
                                                        cb2.currentRY = activeSectionY + 14
                                                    end
                                    ...
                                    -- Calculate new scroll boundaries based on the lowest visible item
                                    local lastY = 0
                                    for _, b2 in ipairs(btns) do
                                        if b2.tab == currentTab and showSet[b2.bg] then
                                            local bottom = b2.ry + b2.ch
                                            if bottom > lastY then lastY = bottom end
                                        end
                                    end
                                    tabRowY[currentTab] = lastY + 20
                                    local newMax = math.max(0, tabRowY[currentTab] - CONTENT_H() + 20)
                                    tabScroll[currentTab] = clamp(tabScroll[currentTab] or 0, 0, newMax)
                                    
                                    if openDropdown then
                                        uiTargetH = L.H + (#openDropdown.options * openDropdown.ch)
                                    end
                                end
                                break
                            end
                ...
                for _,b in ipairs(btns) do
                    if b.isSlider and b.tab==currentTab and menuOpen then
                        ...
                        if clicking and not wasClicking then
                            if inBox(uiX+b.rx,uiY+b.ry,b.cw,b.ch) and b.bg.Visible then 
                                handleDrag=false
                                b.dragging=true 
                            end
                        end
