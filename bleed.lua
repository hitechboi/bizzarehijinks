...
function UILib.Window(titleA, titleB, gameName)
    ...
    mouse.WheelForward:Connect(function() _scrollDelta = _scrollDelta - 1 end)
    mouse.WheelBackward:Connect(function() _scrollDelta = _scrollDelta + 1 end)
    local PAD = 10
    ...
    local function recalculateLayout(tname)
        local currentY = 10 
        local lastHeaderY = 0
        for _, b in ipairs(btns) do
            if b.tab == tname then
