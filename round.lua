...
function UILib.Window(titleA, titleB, gameName)
    ...
    local function restoreFullMenu()
        ...
        -- Don't show tab content yet - let it reveal as the window grows
        uiCurrentH = L.MINI_H
        uiTargetH = L.H
        updatePos()
        menuOpen=true; menuToggledAt=tick()-FADE_DUR-0.01
        -- Show content but hide items that don't fit at current height
        showTab(currentTab)
    ...
    recalculateLayout = function(tname)
        ...
        tabRowY[tname] = lastY + 20
        local newMax = math.max(0, (tabRowY[tname] or 0) - CONTENT_H() + 20)
            ...
            if not minimized then
                ...
                for _,b in ipairs(btns) do
                    if b.currentRY ~= nil and b.tab==currentTab then
                        if b._collapsing and b._collapseTarget then
                            -- snap to target immediately and hide
                ...
                if math.abs(uiCurrentH - uiTargetH) > 0.5 then
                    ...
                    for _,b in ipairs(btns) do
                        if b.tab == currentTab then
                            local isCollapsed = b.section and _collapseSections[b.section]
                            local itemY = uiY + (b.currentRY or b.ry)
                            if itemY + 4 > contentBottom or isCollapsed then
                    ...
                    for _,t in ipairs(tabObjs) do
                        ...
                        if tabY + 26 > contentBottom then
                            t.bg.Visible=false; t.acc.Visible=false
                            t.lbl.Visible=false; t.lblG.Visible=false
                        else
                            t.bg.Visible=true; t.acc.Visible=true
                            t.lbl.Visible=t.sel; t.lblG.Visible=not t.sel
                        end
                    end
                else
                    ...
                    -- re-show items and tabs only once when resize finishes
                    if _wasResizing and uiCurrentH == L.H then
                        _wasResizing = false
                        for _,b in ipairs(btns) do
                            if b.tab == currentTab then
                                local isCollapsed = b.section and _collapseSections[b.section]
                                if not isCollapsed then bShow(b, true); bPos(b) end
                            end
                        end
                        for _,t in ipairs(tabObjs) do
                            setShow(t.bg,true); setShow(t.acc,true)
                ...
                pcall(function()
                    ...
                    if _scrollDelta ~= 0 and inBox(uiX+L.SIDEBAR,uiY+L.TOPBAR,L.CONTENT_W,CONTENT_H()) then
                        local sc=(tabScroll[currentTab] or 0)-_scrollDelta*28
